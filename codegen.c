...
#define CMP(arg1) printf("\tcmp\t$0, %s\n", arg1)
#define JMP(arg1) printf("\tje\t.%s", arg1)
...
static void codegen_func(T_func func) {
  ...
  // create a new scope
  T_offset_scope function_scope = create_offset_scope(current_offset_scope);
  // emit the pseudo ops for the function definition
  printf(".text\n");
  printf(".globl %s\n", func->ident);
  printf(".type %s, @function\n", func->ident);
  ...
  // emit a label for the function
  printf("%s:\n", func->ident);
  insert_offset(function_scope, &func->paramlist, 8);
  codegen_decllist(func->decllist);
  emit_prologue(function_scope->stack_size);
  codegen_stmtlist(func->stmtlist);
  codegen_expr(func->returnexpr);
  POP("%rax");
  emit_epilogue();
  current_offset_scope = destroy_offset_scope(function_scope);
}
...
static void codegen_assignstmt(T_stmt stmt) {
  ...
  POP("%rax");
  T_offset offset = lookup_offset_in_all_offset_scopes(current_offset_scope, stmt->assignstmt.left->identexpr);
...
static void codegen_ifstmt(T_stmt stmt) {
  	CMP("%rax");
   	char buf[100];
  	snprintf(buf, 100, "goto_label_%d", label_counter);
  	codegen_stmtlist(stmt->ifstmt.body);
  	printf(".%s:", buf);
}

static void codegen_ifelsestmt(T_stmt stmt) {
  codegen_ifstmt(stmt->ifelsestmt.ifbranch)
}
...
static void codegen_compoundstmt(T_stmt stmt) {
	codegen(stmt->compoundstmt.stmtlist);
}
...
static void codegen_intexpr(T_expr expr) {
  int offset = lookup_offset_in_scope(current_offset_scope, expr->intexpr);
  MOV_FROM_OFFSET(offset, "%rax");
  PUSH("%rax");
}
